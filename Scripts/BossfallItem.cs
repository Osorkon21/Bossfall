...
namespace BossfallMod.Items
{
    ...
    /// </summary>
    public static class BossfallItem
    {
        #region Fields
        ...
        // Call GenerateLoot method from BossfallOnTabledLootSpawned - this will never have an incoming enemyLevelModifier as it's
        // only going to be used when generating items for location loot piles, never for enemies (in current implementation plans)
        ...
        static DaggerfallUnityItem[] GenerateRandomLoot(LootChanceMatrix matrix, PlayerEntity playerEntity, int enemyLevelModifier = 0)
        {
            ...
            // Route method calls to use ur custom Loot Matrixes (or equivalent) that better customize loot
            // depending on location and enemy type
        ...
        // DELETE AFTER IMPLEMENTATION
        // Stop bosses from using AssignEnemyStartingEquipment to get loot that scales w/enemy level, make 'em use a different
        // method (or from a BossfallLootTable up top) that has more random armor pieces but is just as rewarding as v1.3 boss loot,
        // give 'em lots o' gold & high chance for magic items, gems, good ingredients (and religious items?)

        // DELETE AFTER IMPLEMENTATION
        // Make AssignEnemyStartingEquipment check for existence of modded wpns/armor, if present give 'em chance to have either
        // a vanilla weapon or armor piece or a modded weapon or armor piece (only give 'em one or the other, not both)

        // DELETE AFTER IMPLEMENTATION
        // Make separate StartingEquipment methods for each enemy, call 'em from BossfallEventHandlers
        ...
        public static void AssignEnemyStartingEquipment(PlayerEntity player, EnemyEntity enemyEntity, int variant)
        {
            ...
            if (variant == 0)
            {
                ...
                // I didn't like how often enemies were generated with full suits of armor, so I halved item generation chances.
                chance = 25;
    ...
    /// <summary>
    /// Contains overrides for vanilla item generation methods.
    /// </summary>
    public class BossfallItemOverrides : DaggerfallUnityItem
    {
        /// <summary>
